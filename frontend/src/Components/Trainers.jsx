      ...
      try {
        ...
        const trainersData = await Promise.all(
          staffSnap.docs.map(async (docSnap) => {
            ...
            // 🔥 fetch image from sub-collection
            const docsRef = collection(db, "staff", docSnap.id, "documents");
            const photoQuery = query(docsRef, where("type", "==", "photo"));
            const photoSnap = await getDocs(photoQuery);

            let image = "/images/trainer-placeholder.png"; // fallback

            if (!photoSnap.empty) {
              image = photoSnap.docs[0].data().file;
            }
